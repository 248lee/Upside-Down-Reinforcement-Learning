...
# %%
def warmup(bf, buffer):
    #initial command
    init_desired_reward = 1 
    init_time_horizon = 1 

    for i in range(n_warm_up_episodes):
        desired_return = torch.tensor([float(init_desired_reward)])
        desired_time_horizon = torch.tensor([float(init_time_horizon)])
        state, _ = env.reset()
        states = []
        actions = []
        rewards = []
        while True:
            ...
            desired_time_horizon = torch.tensor([np.maximum(desired_time_horizon, 1).item()])
